...
public class BoardView {
	...
	public void login() {
		...
		while (true) {
			...
			int loginStatus = bc.login(id, pwd);
			
			if (loginStatus == 1) { // 로그인 성공
	...
	public void selectBoardList() {
		...
		if (list.isEmpty()) {
			...
			for (int i = 0; i < list.size(); i++) {
				System.out.println(list.get(i).getBno() + "\t\t" + list.get(i).getTitle() + "\t\t" + list.get(i).getWriter() + "\t\t" + list.get(i).getCreateDate());
			}
		}
	}
	...
	public void insertBoard() {
		...
		int updateCount = bc.insertBoard(title, content);
	...
	public void updateBoard() {
		System.out.print("수정할 게시글 제목 번호: ");
		int bno = sc.nextInt();
		sc.nextLine();
		
		System.out.print("수정할 내용: ");
		String contentEdit = sc.nextLine();
		
		int updateCount = bc.updateBoard(bno, contentEdit);
	...
	public void deleteBoard() {
		...
		int bnoDelete = sc.nextInt();
		sc.nextLine();
		
		int updateCount = bc.deleteBoard(bnoDelete);
